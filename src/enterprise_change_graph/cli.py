...
from .explain import explain_non_impact
...
def _impact(graph, args):
    return analyze_impact(
        ...
        change_kind=getattr(args, "change_kind", None),
        max_depth=getattr(args, "max_depth", None),
        include_relations=getattr(args, "include_relation", []),
        exclude_relations=getattr(args, "exclude_relation", []),
        include_node_types=getattr(args, "include_type", []),
        exclude_node_types=getattr(args, "exclude_type", []),
    )
...
def _policy_from_args(args) -> GatePolicy:
    ...
    overrides = {}
    mapping = {
        "max_affected": "max_affected_nodes",
        "min_tests": "min_tests",
        "min_owners": "min_owners",
        "fail_on_criticality": "fail_on_criticality",
        "max_untested": "max_untested_nodes",
        "max_unowned": "max_unowned_nodes",
    }
    for arg_name, field_name in mapping.items():
        value = getattr(args, arg_name, None)
        if value is not None:
            overrides[field_name] = value
    if getattr(args, "forbid_node", None) is not None:
        overrides["forbid_node_ids"] = tuple(sorted(set(args.forbid_node)))
    if getattr(args, "forbid_type", None) is not None:
        overrides["forbid_node_types"] = tuple(sorted(set(args.forbid_type)))
    if getattr(args, "require_complete", False):
        overrides["require_complete_traversal"] = True
...
def _parser() -> argparse.ArgumentParser:
    ...
    impact = sub.add_parser("impact", help="Traverse impact from a declared change or seed nodes.")
    impact.add_argument("graph")
    _add_selector(impact)
    _add_filters(impact)
    impact.add_argument("--format", choices=("text", "json"), default="text")
    impact.add_argument("--explain", action="store_true")

    why = sub.add_parser("why-not", help="Explain deterministically why a target is not in the impact set.")
    why.add_argument("graph")
    _add_selector(why)
    _add_filters(why)
    why.add_argument("--target", required=True)
    why.add_argument("--format", choices=("text", "json"), default="text")

    gate = sub.add_parser("gate", help="Evaluate deterministic governance checks against impact.")
    gate.add_argument("graph")
    _add_selector(gate)
    _add_filters(gate)
    _add_gate_policy_args(gate)
    gate.add_argument("--format", choices=("text", "json"), default="text")

    dot = sub.add_parser("dot", help="Render Graphviz DOT.")
    dot.add_argument("graph")
    dot.add_argument("--change")

    diff = sub.add_parser("diff", help="Compare graph documents and derive change seed candidates.")
    diff.add_argument("before")
    diff.add_argument("after")
    diff.add_argument("--format", choices=("text", "json"), default="text")

    review = sub.add_parser("review", help="Removal-aware before/after impact review.")
    review.add_argument("before")
    review.add_argument("after")
    _add_filters(review)
    review.add_argument("--format", choices=("text", "json", "markdown"), default="text")
    review.add_argument("--output")

    compose = sub.add_parser("compose", help="Compose multiple graph fragments with provenance.")
    compose.add_argument("graphs", nargs="+")
    compose.add_argument("--namespace", action="append", default=[], metavar="PATH=PREFIX")
    compose.add_argument("--output", required=True)

    csv_cmd = sub.add_parser("import-csv", help="Import Nodes/Edges/Changes CSV catalogs.")
    csv_cmd.add_argument("nodes")
    csv_cmd.add_argument("--edges")
    csv_cmd.add_argument("--changes")
    csv_cmd.add_argument("--output", required=True)

    xlsx_cmd = sub.add_parser("import-xlsx", help="Import an Excel workbook with Nodes/Edges/Changes sheets.")
    ...
    report = sub.add_parser("report", help="Generate a deterministic impact report.")
    report.add_argument("graph")
    _add_selector(report)
    _add_filters(report)
    _add_gate_policy_args(report)
    report.add_argument("--format", choices=("markdown", "html", "json"), default="markdown")
    report.add_argument("--output")

    release = sub.add_parser("release", help="Analyze a bundle of changes and detect collisions.")
    release.add_argument("graph")
    release.add_argument("--change", action="append", required=True)
    release.add_argument("--max-depth", type=int, default=None)
    release.add_argument("--format", choices=("text", "json"), default="text")

    observe = sub.add_parser("observe", help="Compare predicted impact with an observed historical record.")
    observe.add_argument("graph")
    observe.add_argument("--change", required=True)
    observe.add_argument("--history", required=True)
    observe.add_argument("--record", help="History record change id; defaults to --change.")

    similar = sub.add_parser("similar", help="Find previous changes with overlapping affected subgraphs.")
    similar.add_argument("graph")
    similar.add_argument("--change", required=True)
    similar.add_argument("--history", required=True)
    similar.add_argument("--limit", type=int, default=5)

    context = sub.add_parser("context", help="Emit compact agent/MCP-ready change context as JSON.")
    context.add_argument("graph")
    _add_selector(context)
    _add_filters(context)
    _add_gate_policy_args(context)

    export = sub.add_parser("export", help="Export the graph for other graph tools.")
    export.add_argument("graph")
    export.add_argument("--format", choices=("graphml", "cypher"), required=True)
    export.add_argument("--output")

    explore = sub.add_parser("explore", help="Generate a dependency-free static HTML graph explorer.")
...
def _handle_preload(args) -> int | None:
    if args.command == "diff":
        ...
        if args.format == "json":
            ...
            print("Graph diff")
            print(f"Nodes: added={summary['added_nodes']}, removed={summary['removed_nodes']}, modified={summary['modified_nodes']}")
            print(f"Edges: added={summary['added_edges']}, removed={summary['removed_edges']}, modified={summary['modified_edges']}")
            print(f"Changes: added={summary['added_changes']}, removed={summary['removed_changes']}, modified={summary['modified_changes']}")
            print(f"Impact seeds in before graph: {', '.join(diff.impact_seeds_before) or 'none'}")
            print(f"Impact seeds in after graph: {', '.join(diff.impact_seeds_after) or 'none'}")
            print(f"Removed-node seed candidates: {', '.join(diff.removed_seed_candidates) or 'none'}")
        return 0
    ...
    if args.command == "review":
        review = analyze_diff_impact(
            ...
            include_node_types=args.include_type,
            exclude_node_types=args.exclude_type,
        )
        if args.format == "json":
            text = json.dumps(review.to_dict(), indent=2, sort_keys=True) + "\n"
        elif args.format == "markdown":
            text = render_review_report(review)
        else:
            text = (
                f"Diff impact: affected={len(review.affected_node_ids)}, tests={len(review.regression_tests)}, "
                f"owners={len(review.owners)}, max-criticality={review.max_criticality or 'none'}\n"
                f"Before seeds: {', '.join(review.diff.impact_seeds_before) or 'none'}\n"
                f"After seeds: {', '.join(review.diff.impact_seeds_after) or 'none'}\n"
            )
    ...
    if args.command == "compose":
        ...
        for raw in args.namespace:
            if "=" not in raw:
                raise ValueError("--namespace must use PATH=PREFIX")
            path, prefix = raw.split("=", 1)
            if not path or not prefix:
                raise ValueError("--namespace must use non-empty PATH=PREFIX")
            namespaces[path] = prefix
        graph = compose_graphs(args.graphs, namespaces=namespaces)
        dump_graph(graph, args.output)
        print(f"OK: composed nodes={len(graph.nodes)}, edges={len(graph.edges)}, changes={len(graph.changes)} -> {args.output}")
        return 0

    importers = {
        "import-csv": lambda: import_catalog_csv(args.nodes, edges_path=args.edges, changes_path=args.changes),
        "import-xlsx": lambda: import_catalog_workbook(args.workbook),
        "import-interface": lambda: import_interface_as_code(args.source),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "validate":
            print(f"OK: version={graph.version}, nodes={len(graph.nodes)}, edges={len(graph.edges)}, changes={len(graph.changes)}, relation-rules={len(graph.relation_rules)}")
            return 0

        if args.command == "impact":
            result = _impact(graph, args)
            print(json.dumps(result.to_dict(), indent=2, sort_keys=True) if args.format == "json" else render_text(result, explain=args.explain))
            return 0

        if args.command == "why-not":
            result = _impact(graph, args)
            explanation = explain_non_impact(graph, result, args.target, max_depth=args.max_depth)
            if args.format == "json":
                print(json.dumps(explanation.to_dict(), indent=2, sort_keys=True))
            else:
                print(f"Target: {explanation.target_id}")
                print(f"Impacted: {'yes' if explanation.impacted else 'no'}")
                print(f"Reason: {explanation.reason}")
                if explanation.path:
                    print("Physical path: " + " -> ".join(explanation.path))
                if explanation.barrier_at:
                    print(f"Barrier at: {explanation.barrier_at}")
                if explanation.barrier_relation:
                    print(f"Relation: {explanation.barrier_relation}")
                if explanation.barrier_detail:
                    print(f"Detail: {explanation.barrier_detail}")
            return 0

        if args.command == "gate":
            impact = _impact(graph, args)
            coverage = assess_coverage(graph, impact)
            policy = _policy_from_args(args)
            gate = evaluate_gate(impact, coverage=coverage, **policy.to_dict())
            if args.format == "json":
                print(json.dumps({"policy": policy.to_dict(), **gate.to_dict()}, indent=2, sort_keys=True))
            elif gate.passed:
                print(f"PASS: affected={len(impact.impacted)}, tests={len(impact.regression_tests)}, owners={len(impact.owners)}, untested={len(coverage.untested_nodes)}, unowned={len(coverage.unowned_nodes)}, max-criticality={impact.max_criticality or 'none'}")
            else:
        ...
        if args.command == "dot":
            ...
            if args.change:
                highlighted = {node.id for node in analyze_impact(graph, change_id=args.change).impacted}
        ...
        if args.command == "quality":
            ...
            if args.format == "json":
                print(json.dumps(quality.to_dict(), indent=2, sort_keys=True))
            else:
                print("Graph quality diagnostics")
                for key, value in quality.to_dict()["summary"].items():
                    print(f"- {key}: {value}")
                if quality.high_criticality_without_tests:
                    print("High/critical without tests: " + ", ".join(quality.high_criticality_without_tests))
                if quality.high_criticality_without_owners:
                    print("High/critical without owners: " + ", ".join(quality.high_criticality_without_owners))
            return 0
        ...
        if args.command == "report":
            ...
            has_policy = (
                args.policy
                or any(getattr(args, name, None) is not None for name in ("max_affected", "min_tests", "min_owners", "max_untested", "max_unowned", "fail_on_criticality"))
                or args.forbid_node is not None
                or args.forbid_type is not None
            ...
            if args.format == "json":
                ...
                markdown = render_impact_report(impact, coverage=coverage, gate=gate)
                text = render_html(markdown) if args.format == "html" else markdown
            _write_or_print(text, args.output)
            return 0 if not gate or gate.passed else 3
        ...
        if args.command == "release":
            ...
            if args.format == "json":
                ...
                print(f"Release changes: {len(release.change_ids)}")
                print(f"Collision nodes: {len(release.collisions)}")
                for node, changes in release.collisions.items():
                    print(f"- {node}: {', '.join(changes)}")
                print("Regression tests: " + (", ".join(release.regression_tests) or "none"))
                print("Approval owners: " + (", ".join(release.owners) or "none"))
            return 0

        if args.command == "observe":
            impact = analyze_impact(graph, change_id=args.change)
            records = load_history(args.history)
            record_id = args.record or args.change
            record = next((item for item in records if item.change_id == record_id), None)
            if record is None:
                raise KeyError(f"history record not found: {record_id}")
            print(json.dumps(compare_prediction(impact, record.affected_nodes).to_dict(), indent=2, sort_keys=True))
            return 0

        if args.command == "similar":
            impact = analyze_impact(graph, change_id=args.change)
            matches = find_similar_changes(impact, load_history(args.history), limit=args.limit)
        ...
        if args.command == "context":
            ...
            policy = _policy_from_args(args)
            gate = evaluate_gate(impact, coverage=coverage, **policy.to_dict())
            print(json.dumps(build_agent_context(impact, coverage=coverage, gate=gate), indent=2, sort_keys=True))
            return 0

        if args.command == "export":
            text = render_graphml(graph) if args.format == "graphml" else render_cypher(graph)
            _write_or_print(text, args.output)
            return 0

        if args.command == "explore":
            Path(args.output).write_text(render_explorer_html(graph, change_id=args.change), encoding="utf-8")
            print(f"OK: explorer -> {args.output}")
            return 0
